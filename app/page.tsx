      ...
      >
        ...
        <div className="action-row" aria-hidden={winOpen || gameOver || undefined} inert={winOpen || gameOver || undefined}>
          <div className="mini-actions">
            <button className="icon-button" onClick={undo} disabled={!history.length || animating || aiRunning} aria-label={aiRunning ? ui.aiUndoBlocked : ui.undo} data-tooltip={aiRunning ? ui.aiUndoBlocked : ui.undo}><ControlGlyph name="undo" /></button>
            <button className={`icon-button sound-button${musicPlaying ? " is-playing" : ""}`} onClick={toggleSound} aria-label={musicPlaying ? ui.soundOff : ui.soundOn} data-tooltip={musicPlaying ? ui.soundOff : ui.soundOn}><ControlGlyph name={musicPlaying ? "volume" : "mute"} /></button>
            <button className="icon-button" onClick={() => setDark((value) => !value)} aria-label={dark ? ui.light : ui.dark} data-tooltip={dark ? ui.light : ui.dark}><ControlGlyph name={dark ? "sun" : "moon"} /></button>
            <GlassMenu
              compact
              value={language}
              label={ui.language}
            ...
            />
            <button ref={helpButtonRef} className="icon-button" onClick={() => { pauseAi(); setOpenMenu(null); setHelpOpen(true); }} aria-label={ui.help} data-tooltip={ui.help}><ControlGlyph name="help" /></button>
          </div>
