...
export function decideExpert(board: Board, anchor: ExpertCorner, budgetMs: number, id = 0): ExpertDecision & { id: number } {
  ...
  const nodeLimit = Math.max(600, Math.floor(Math.max(20, Math.min(240, budgetMs)) * 48));
  let nodes = 0;
  ...
  const sampleLimit = board.length === 4 ? 8 : 5;
  const scored = root.map(({ direction, moved, movableTiles }, order) => {
    let total = moved.gained * 9 + nTupleScore(moved.board, anchor);
    ...
    for (const cell of cells) {
      for (const tile of [2, 4]) {
        ...
        let bestFollow = nTupleScore(spawned, anchor);
        for (const follow of legalMoves(spawned)) {
          bestFollow = Math.max(bestFollow, nTupleScore(follow.moved.board, anchor) + follow.moved.gained * 8);
          nodes += 1;
          if (nodes >= nodeLimit) break;
        }
        total += (tile === 2 ? 0.9 : 0.1) * bestFollow / Math.max(1, cells.length);
        nodes += 1;
        if (nodes >= nodeLimit) break;
      }
      if (nodes >= nodeLimit) break;
    }
    return { direction, score: total - order * 0.001, movableTiles };
  });
