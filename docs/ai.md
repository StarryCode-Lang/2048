...
适用基线：`v2.0.0`。本文件描述当前线上默认 Search 与可选 Adaptive 实验路线；算法门槛变化时必须同步 `CHANGELOG.md` 与发布合同。
...
`app/ai/expert.ts` 提供 afterstate N-tuple 风格评估器。它使用多组行列/角落 tuple、蛇形链和空格价值，对有效移动后的局面做确定性分层抽样，再进行一层后继移动比较；终局随机分支会得到明确的失败惩罚，且每个根方向获得相同确定性工作配额。它不接收随机数状态、不读取未来生成结果，也不会修改默认 Search 引擎的缓存或回归基线。

界面中的“自适应专家”会在前期使用 afterstate 评估；当最大块达到 256 或空格不超过 6 时，由 Worker 自动切回更可靠的 Search 残局搜索。旧的本地 `expert` 偏好会迁移到 `adaptive`。它仍是可解释的轻量实验模式，不冒充训练完成的权重模型。

固定 8 种子、1,024 节点预算的当前门槛结果：300 步时自适应引擎 8/8 达到 512，平均分 4,548（Search 为 4,134）；600 步时 8/8 达到 1024，平均分 10,460（Search 为 10,461）。因此 Search 继续作为更快的稳定默认，自适应模式作为已通过生存门槛的可选实验路线。
...
- `tests/ai-adaptive.test.mjs` 固定验证 300 步全部达到 512、600 步全部达到 1024，并拒绝任何提前死亡。
- `npm run benchmark:ai:engines` 使用相同种子和真实传入的节点预算并排运行 Search、纯 Expert 与 Adaptive，输出分数、到达高块数量和耗时差异；它用于研究，不会绕过发布门禁。
